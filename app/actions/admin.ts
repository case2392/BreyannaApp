...
// ---- Link a pre-existing Stripe subscription to a member ----------------

// Connect a Stripe subscription (created directly in Stripe before the site was
// live) to a member's CRM membership, so it renews and stays in sync like a
// normal site signup. Staff paste the subscription id and choose the plan.
export async function linkStripeSubscription(
  ...
  if (!subId.startsWith("sub_"))
    return {
      ok: false as const,
      error: "Enter a valid Stripe subscription ID (it starts with \"sub_\").",
    };

  const plan = await prisma.membershipPlan.findUnique({ where: { id: planId } });
  if (!plan) return { ok: false as const, error: "Choose a plan." };
  ...
  if (linked && linked.userId !== userId)
    return {
      ok: false as const,
      error: "That subscription is already linked to a different member.",
    };
  ...
  try {
    ...
    return {
      ok: false as const,
      error: "Couldn't find that subscription in Stripe — double-check the ID.",
    };
  }
  ...
  const end =
    (sub as any).current_period_end ??
    (sub as any).items?.data?.[0]?.current_period_end;
  const periodEnd = end ? new Date(end * 1000) : undefined;
  const autoRenew = active && !sub.cancel_at_period_end;
  const expiresAt =
    periodEnd ??
    (() => {
      const d = new Date();
  ...
  if (linked && linked.userId === userId) {
    ...
    const candidate = await prisma.membership.findFirst({
      where: { userId, planId, stripeSubscriptionId: null, status: "ACTIVE" },
  ...
  // Backfill the Stripe customer id so future webhooks match this member directly.
  const customerId =
